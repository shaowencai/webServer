...
static void timeout_cb(struct webnet_session *session)
{
 ...
 */
struct webnet_session* webnet_session_create(int handle)
{
    ...
    if (session != NULL)
    {
        ...
        timer_init(&session->timer,timeout_cb,session,60,0);
...
int webnet_session_read(struct webnet_session *session, char *buffer, int length)
{
    ...
    /* Read some data */
#if WEBNET_USING_HTTPS
    read_count = mbedtls_server_read( session->socket,(unsigned char *) buffer, length );
#else
...
#if WEBNET_USING_HTTPS
    mbedtls_server_write(session->socket, session->buffer, length );
#else
...
int webnet_session_write(struct webnet_session *session, const uint8_t* data, size_t size)
{
    /* send data directly */
#if WEBNET_USING_HTTPS
    mbedtls_server_write(session->socket, data, size );
#else
...
{
    struct webnet_session *session;
    //struct webnet_session *next_session;
    
    session = _session_list;
    ...
    for (;;)
    {
        ...
        if (mbedtls_server_read( session->socket, session->buffer, 1 ) > 0)
        {
        	session->buffer_offset += 1;
            timer_reset(&session->timer);
            if (session->session_ops == NULL)
            {
                struct webnet_request *request;
