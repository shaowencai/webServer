...
#define MULTIPAFORM_DATA_STRING                 "multipart/form-data"
...
static char* _webnet_module_upload_parse_header(struct webnet_session* session, char* buffer, size_t length)
{
    char *ptr, *ch, *end_ptr;
...
static char* _next_possible_boundary(struct webnet_session* session, char* buffer, size_t length)
{
    char *ptr;
    struct webnet_module_upload_session *upload_session;
...
static void _handle_section(struct webnet_session* session, char* buffer, size_t length)
{
    char *ptr;
    struct webnet_module_upload_session *upload_session;
        ...
        else
        {
                    ...
                    upload_session->content_type != NULL)
            {
                if (upload_session->file_opened == 0)
                {
                    ...
                    if((int)(upload_session->user_data) < 0)
                    {
                        session->session_phase = WEB_PHASE_CLOSE;
                        return;
                    }
